...
module.exports = {
  ...
  create: function(req, res, next) {
    ...
    // findOneEmail() is a dynamic finder in that it searches the model by a particular attribute
    User.findOneByEmail(req.param('email'), function foundUser(err, user) {
      ...
      require('bcrypt').compare(req.param('password'), user.encryptedPassword, function(err, valid) {
        ...
        // if the password from the form is not match the password from the database...
        if (!valid) {
        ...
        // if the user also an admin redirect to the user list (e.g. /views/user/index.ejs)
